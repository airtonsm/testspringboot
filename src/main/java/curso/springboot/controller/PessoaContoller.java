...
public class PessoaContoller {
	...
	public ModelAndView salvar(@Valid Pessoa pessoa, 
		...
		if(file.getSize() > 0 ) {/*cadastrando um curriculo*/
			...
			if(pessoa.getId() != null && pessoa.getId() > 0) { //editando
				byte[] pessoaTemp = pessoaRepository.
						findById(pessoa.getId()).get().getCurriculo(); // recarregar curriculo do banco
				pessoa.setCurriculo(pessoaTemp);
			}
		}
	...
	public void imprimePdf(@RequestParam("nomepesquisa") String nomepesquisa, @RequestParam("pesqsexo") String pesqsexo,
		...
		/* Chamar o serviço que faz a geração de relatorio */
		byte[] pdf = reportUtil.gerarRelatorio(pessoas, "pessoa", request.getServletContext());

		/* Tamanho da resposta */
		response.setContentLength(pdf.length);

		/* Definir da resposta o tipo do arquivo */
		response.setContentType("application/octet-stream");

		/* Cabeçalho da nossa resposta */
	 ...
	 * 
	 * 
	 * @GetMapping("/editartelefone/{telefoneid}") public ModelAndView
	 * editarTelefone(@PathVariable("telefoneid") Long telefoneid) {
	 * 
	 * Optional<Telefone> telefone = telefoneRepository.findById(telefoneid);
	 * 
	 * ModelAndView modelAndView = new ModelAndView("cadastro/telefones");
	 * modelAndView.addObject("telefoneobj", telefone.get()); return modelAndView; }
	 */
